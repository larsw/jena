...
/** Size limits for SPARQL query/update request text. */
final class SPARQLRequestSize {
    static final String SYSTEM_PROPERTY_MAX_REQUEST_SIZE = "jena.fuseki.sparql.maxRequestSize";
    static final long DEFAULT_MAX_REQUEST_SIZE = 64L * 1024L * 1024L;

    private SPARQLRequestSize() {}
    ...
    static void rejectIfContentLengthExceeds(HttpAction action) {
        ...
        if ( isEnabled(limit) && len > limit )
            tooLarge(format("SPARQL request Content-Length %d exceeds configured limit %d", len, limit));
    }
    ...
    static void rejectIfStringExceeds(String string, String description) {
        long limit = maxRequestSize();
        if ( ! isEnabled(limit) || string == null )
            return;
        long len = string.getBytes(StandardCharsets.UTF_8).length;
        if ( len > limit )
            tooLarge(format("%s size %d exceeds configured limit %d", description, len, limit));
    }
